...
//These are the reflections of the Options property of the CompletionProposal
  ...
  protected
    ...
    procedure WMEraseBackgrnd(var Message: TMessage); message LM_ERASEBKGND;
...
const
  ...
  { Utility routines }
  function DisplayCoord(AColumn, ARow: Integer): TPoint;
  begin
  ...
  function BufferCoord(AChar, ALine: Integer): TPoint;
  begin
    Result.X := AChar;
    Result.Y := ALine;
  end;
...
function PaintChunks(TargetCanvas: TCanvas; const Rect: TRect;ChunkList: TFormatChunkList; Columns: TProposalColumns; Images: TImageList;
...
begin
  Chunks := TFormatChunkList.Create;
  try
    TmpRect := Classes.Rect(0, 0, MaxInt, MaxInt);
...
{$ENDIF}
  ...
  FScrollbar.TabStop := False;//not work ok
...
begin
  ...
  //remplazaado .AddFocusControl(Self);
end;
...
begin
  ...
  begin
    case Key of
      VK_RETURN:
        if (FCompleteWithEnter) and Assigned(OnValidate) then   begin
             OnValidate(Self, Shift, #0); //GBN 15/11/2001
             HandleKey:=true;
        end;

      VK_TAB:
        if  (FCompleteWithTab) and Assigned(OnValidate) then begin
      ...
      VK_ESCAPE:
      begin
        if Assigned(OnCancel) then  begin
          OnCancel(Self);
        end;
      end;
      VK_LEFT:
      ...
      VK_DELETE: if Assigned(CurrentEditor) then  begin
    ...
    if HandleKey and Visible then begin
       Key:=0;
    end;

    if  (HandleKey) and (Key = VK_RETURN)   then Key:=0;
...
begin
  //CurrentEditor.Parent.Caption:= IntToStr(ord(Key));
  if DisplayType = ctCode then
  begin
    case Key of
      ...
      #8:
        ...
        with CurrentEditor as TCustomSynEdit do
          CommandProcessor(ecChar, Key, nil);
           CurrentEditor.Parent.Caption:=Key;  //test mio
        if Assigned(OnCancel) then
          OnCancel(Self);
    end;
  end;
...
begin
  ...
  begin
      ...
      begin
        ...
        if Assigned(OnPaintItem) then
          OnPaintItem(Self, LogicalToPhysicalIndex(FScrollBar.Position+i),
            Canvas, Classes.Rect(0, FEffectiveItemHeight*i, ClientWidth - FScrollbar.Width,
        ...
        begin
          ...
          begin
            FormattedTextOut(Canvas, Classes.Rect(FMargin,
    ...
    begin
      ...
      begin
        ...
        TmpRect := Classes.Rect(0, 0, ClientWidth+1, FHeightBuffer);                        //GBN
  ...
  begin
    ...
    begin
      ResetCanvas;
      tmpRect := Classes.Rect(0, 0, ClientWidth, ClientHeight);
      Canvas.FillRect(tmpRect);
      //Frame3D(Canvas, tmpRect, cl3DLight, cl3DDkShadow, 1);
      //Canvas.Frame3d(tmpRect, cl3DLight, bvRaised);
      ...
      begin
        AlreadyDrawn := False;
        if Assigned(OnPaintItem) then
          OnPaintItem(Self, i, Canvas, Classes.Rect(0, FEffectiveItemHeight* i +FMargin,
        ...
        begin
          ...
          FormattedTextOut(Canvas, Classes.Rect(FMargin+1,
...
begin
  //CurrentEditor.Parent.Caption:='Scrolling';
  with CurrentEditor as TCustomSynEdit do
  begin
    SetFocus;
    //This tricks the caret into showing itself again.
    //AlwaysShowCaret := False; comentado
    //AlwaysShowCaret := True;  comentado
//    UpdateCaret;
  end;
end;
...
procedure TSynBaseCompletionProposalForm.WMActivate (var Message : TLMActivate);
var
  ParentForm: TSynForm;
begin
  ...
  if Owner.Owner is TSynForm then  begin
  ...
  if Assigned(ParentForm) and ParentForm.HandleAllocated then
    ...
    SendMessage(ParentForm.Handle, LM_NCACTIVATE, 1, 0);
...
begin
  ...
  begin
 //   (CurrentEditor as TCustomSynEdit).AlwaysShowCaret := OldShowCaret; comentado
//    (CurrentEditor as TCustomSynEdit).UpdateCaret;
    if DisplayType = ctCode then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      //AlwaysShowCaret := Focused; comentado
//      UpdateCaret;
    end;
  end;
  //GBN 28/08/2002
...
procedure TSynBaseCompletionProposalForm.WMGetDlgCode(var Message: TLMNoParams);
begin
  inherited;
...
begin
  ...
  begin
    if FTitle <> '' then
      ...
      if Assigned(FScrollbar) then      //if remove cause EAccessViolation
...
begin
  ...
  ctCode:
    begin
      CurrentString := s;
      //TWinControl(Form.CurrentEditor.Owner).Caption:=s;
    ...
    begin
      ...
      try
        if FAdjustCompletionStart then
          FCompletionStart := CaretX;
        BlockBegin := BufferCoord(FCompletionStart, CaretY);
        //p.y :=CaretY;
        //p.x :=CaretX;
        if EndToken = #0 then begin
          //BlockEnd := BufferCoord(WordEnd.X, CaretY) comentado revisar luego
        ...
        if SelText <> Value then
          SelText := Value;
          //TextBetweenPointsEx[BlockBegin, NewBlockEnd, scamEnd] := Value;
...
begin
  ...
  begin
    ...
    //hook en windows trabaja sin esto
    if (DisplayType = ctCode) and Form.Visible then begin
       Form.KeyDown(Key,Shift);
    end;

  end;
  //Take from syncompletion
...
begin
  ...
  if i <> -1 then begin
    ...
    command:= ecChar;
    Handled:=true;
    HookedEditorCommand((Sender as TCustomSynEdit),True,Handled,command,Key,NIL,NIL);
  end;
end;
...
//  if not Assigned(fEditors) then exit;
  ...
  if i = -1 then begin
    ...
    if not (csDesigning in ComponentState) then begin
      ...
      aEditor.OnKeyPress := EditorKeyPress;
      //aEditor.OnUTF8KeyPress := EditorUTF8KeyPress;
      TSynEdit(aEditor).Options:=TSynEdit(aEditor).Options+[eoPersistentCaret];
    end;
    //AEditor.AddKeyDownHandler(EditorKeyDown);
    //AEditor.AddKeyPressHandler(EditorKeyPress);
    //AEditor.RegisterCommandHandler(HookedEditorCommand,Self); anadir controladores de eventos
  end;
end;
    ...
    begin
      ...
      begin
        if DefaultType = ctHint then
          ...
          p := ClientToScreen(Classes.Point(CaretXPix, CaretYPix + LineHeight + 1));
          //Inc(p.y, LineHeight);
        end;
        ...
        CopyStringToCharSet(EndOfTokenChr, Form.FWordBreakChars);
         // ShowMessage(GetPreviousToken(AEditor));
        FPreviousToken := GetPreviousToken(Form.CurrentEditor as TCustomSynEdit);
        //ShowMessage(FPreviousToken);
        //ExecuteEx(GetCurrentInput(AEditor), p.x, p.y, DefaultType); //org
        ExecuteEx(FPreviousToken, p.x, p.y, DefaultType);
        //ExecuteEx(GetPreviousToken(AEditor), p.x, p.y-1,DefaultType);
...
begin
  ...
  begin
    ...
    ctCode:
      begin
          if AChar = #27 then CancelCompletion;
...
begin
  ...
  try
    ...
    if (i <> -1) then
    begin
{comentado estas opciones no estan disponibles en el synedit de lazarus}
...
//      Editor.UndoList.AddChange(crAutoCompleteBegin, StartOfBlock, StartOfBlock, '',   smNormal);
{comentado}
...
{comentado  estas opciones no estan disponibles en el synedit de lazarus}
      //BeginningSpaceCount := Editor.DisplayX - 1;  //GBN 2002/04/24 comentado  DisplayX no existe en el synedit de lazarus
...
{cometado estas opciones no estan disponibles en el synedit de lazarus}
//      if ChangedIndent or ChangedTrailing then Editor.Options := OrigOptions;

//      Editor.UndoList.AddChange(crAutoCompleteEnd, StartOfBlock, StartOfBlock, '', smNormal);
{comentado}
