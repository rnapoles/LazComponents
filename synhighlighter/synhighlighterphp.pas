...
const
  CountPhpCodeFoldBlockOffset: Pointer =
    Pointer(PtrInt(Integer(high(TPhpCodeFoldBlockType))+1));

type

  { TSynPHPSyn }

  TSynPHPSyn = class(TSynCustomFoldHighlighter)
  private
    fRange: TRangeState;
    fLine: PChar;
    {$IFDEF SYN_LAZARUS}
  ...
  protected
    ...
    function StartPhpCodeFoldBlock(ABlockType: TPhpCodeFoldBlockType): TSynCustomCodeFoldBlock;
    procedure EndPhpCodeFoldBlock;
    function TopPhpCodeFoldBlockType(DownIndex: Integer = 0): TPhpCodeFoldBlockType;
...
begin
  StartPhpCodeFoldBlock(cfbtPhpBrace);
  inc(Run);
  fTokenId := tkSymbol;
end;
  ...
  case FLine[Run + 1] of
    ...
    '*':
      begin
        ...
        StartPhpCodeFoldBlock(cfbtPhpComents);
        inc(Run);
        while fLine[Run] <> #0 do
          case fLine[Run] of
            '*':
...
procedure TSynPHPSyn.CreateRootCodeFoldBlock;
begin
  inherited CreateRootCodeFoldBlock;
  RootCodeFoldBlock.InitRootBlockType(Pointer(PtrInt(cfbtPhpNone)));
end;

function TSynPHPSyn.StartPhpCodeFoldBlock(ABlockType: TPhpCodeFoldBlockType): TSynCustomCodeFoldBlock;
var
  FoldBlock: Boolean;
  p: PtrInt;
begin
  ...
  if not FoldBlock then
    p := PtrInt(CountPhpCodeFoldBlockOffset);
  Result := StartCodeFoldBlock(p + Pointer(PtrInt(ABlockType)), FoldBlock);
end;

procedure TSynPHPSyn.EndPhpCodeFoldBlock;
var
  DecreaseLevel: Boolean;
begin
  DecreaseLevel := TopCodeFoldBlockType < CountPhpCodeFoldBlockOffset;
  EndCodeFoldBlock(DecreaseLevel);
end;

function TSynPHPSyn.TopPhpCodeFoldBlockType(DownIndex: Integer): TPhpCodeFoldBlockType;
var
  p: Pointer;
begin
  p := TopCodeFoldBlockType(DownIndex);
  if p >= CountPhpCodeFoldBlockOffset then
    p := p - PtrUInt(CountPhpCodeFoldBlockOffset);
  Result := TPhpCodeFoldBlockType(PtrUInt(p));
end;

function TSynPHPSyn.GetFoldConfigInstance(Index: Integer): TSynCustomFoldConfig;
begin
  Result := inherited GetFoldConfigInstance(Index);
  Result.Enabled := True;
end;

function TSynPHPSyn.GetFoldConfigCount: Integer;
begin
  // excluded cfbtLfmNone
  Result := ord(high(TPhpCodeFoldBlockType)) - ord(low(TPhpCodeFoldBlockType));
end;

function TSynPHPSyn.GetFoldConfigInternalCount: Integer;
begin
  // include cfbtLfmNone
  Result := ord(high(TPhpCodeFoldBlockType)) - ord(low(TPhpCodeFoldBlockType)) + 1;
end;
